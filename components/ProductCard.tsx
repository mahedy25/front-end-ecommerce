  ...
  return (
    <div className="w-full max-w-xs border border-[#046C4E]/20 rounded-md p-3 sm:p-4 bg-white flex flex-col">
      ...
      <div className="mt-3 text-sm text-gray-600">
        ...
        <h2 className="text-gray-900 font-semibold text-lg truncate">{product.title}</h2>
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            ...
            >
              <path
                ...
                fill={i <= (product.rating ?? 4) ? '#046C4E' : '#046C4E40'}
              />
            </svg>
          ))}
        </div>
        ...
        <div className="flex justify-between items-end mt-4">
          ...
          {count === 0 ? (
            ...
            <div className="flex items-center gap-2 h-9 bg-[#046C4E]/10 text-[#046C4E] px-2 rounded-md select-none">
              <button onClick={() => setCount((p) => Math.max(p - 1, 0))} className="px-2">−</button>
              <span className="w-5 text-center">{count}</span>
              <button onClick={() => setCount((p) => p + 1)} className="px-2">+</button>
            </div>
          )}
        </div>
      </div>
